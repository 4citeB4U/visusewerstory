// Re-export authoritative component from the root `components/` folder to avoid parsing the corrupted src copy.
export { default } from '../../components/EvidenceBasedCharts';
                  </LineChart>
                </ResponsiveContainer>
...
// Combined evidence/chart component provided by user

const EVIDENCE_SOURCES: any = {
  zippia: { url: "https://www.zippia.com/visu-sewer-careers-1570669/", date: "2024-03", title: "Zippia Employee Data" },
  hovmsd: { url: "https://hvmsd.org/interceptor-rehabilitation-project", date: "2023-08", title: "HOVMSD Project Documentation" },
  fortpoint: { url: "https://fortpointcapital.com/portfolio/visusewer", date: "2023-11", title: "Fort Point Capital Portfolio" },
  market: { url: "https://www.marknteadvisors.com", date: "2024", title: "Global Wastewater Market Report" },
  pitchbook: { url: "https://pitchbook.com/profiles/company/130816-36", date: "2024", title: "PitchBook Company Profile" },
  import { useState } from 'react';
  import { Area, AreaChart, Bar, BarChart, CartesianGrid, Cell, Legend, Line, LineChart, Pie, PieChart, PolarAngleAxis, PolarGrid, PolarRadiusAxis, Radar, RadarChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
      ...
      <div className="w-full min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white p-6">
        ...
        <div className="max-w-7xl mx-auto mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">Visu-Sewer Financial Intelligence</h1>
              <p className="text-slate-400 mt-2">Evidence-Backed Analysis & AI-Ready Data Structures</p>
            </div>
            <button onClick={() => setShowEvidence(!showEvidence)} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg transition-all">
        ...
        <div className="max-w-7xl mx-auto">
          {selectedChart === 'revenue' && (
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              ...
              {/* Removed narrative text to keep right column focused on charts */}
            </div>
          )}
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                <div className="bg-slate-900/50 p-6 rounded-lg border border-slate-700">
                  ...
                  {/* Source note removed to keep chart panel minimal */}
                </div>
                ...
                <div className="bg-slate-900/50 p-6 rounded-lg border border-slate-700">
                  ...
                  {/* Calculation note removed to keep chart panel minimal */}
                </div>
              </div>
          ...
          {selectedChart === 'pipeline' && (
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              ...
              <div className="mt-6 bg-green-900/20 border border-green-700/50 rounded-lg p-4">
                <strong className="text-green-400">Cost Performance Summary:</strong>
                <div className="grid md:grid-cols-3 gap-4 mt-3">
                  <div className="text-center"><div className="text-2xl font-bold text-green-400">{(((projectPipeline.reduce((sum, p) => sum + Math.abs(p.variance), 0)) / projectPipeline.reduce((sum, p) => sum + p.budgeted, 0)) * 100).toFixed(1)}%</div><div className="text-xs text-slate-400">Average Under Budget</div></div>
                  <div className="text-center"><div className="text-2xl font-bold text-blue-400">${projectPipeline.reduce((sum, p) => sum + p.awarded, 0).toFixed(1)}M</div><div className="text-xs text-slate-400">Total Contract Value</div></div>
                  <div className="text-center"><div className="text-2xl font-bold text-purple-400">{(projectPipeline.reduce((sum, p) => sum + p.completion, 0) / projectPipeline.length).toFixed(0)}%</div><div className="text-xs text-slate-400">Average Completion</div></div>
                </div>
              </div>
            </div>
          )}
          ...
          {selectedChart === 'competitive' && (
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                              <div className="bg-amber-900/20 border border-amber-700/50 rounded-lg p-4"><strong className="text-amber-400">Growth Opportunities:</strong><ul className="mt-2 text-sm text-slate-300 space-y-1"><li>• Scale (65)</li><li>• Technology (85)</li></ul></div>
                            </div>
                          </div>
                        )}
                        ...
                        {selectedChart === 'scenarios' && (
                          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                            ...
                            <div className="grid gap-6 mb-6">
                              {financialScenarios.map((scenario) => (
                                <div key={scenario.scenario} className="bg-slate-900/50 p-6 rounded-lg border border-slate-700">
                                  <div className="flex items-start justify-between mb-4"><div><h3 className="text-xl font-semibold text-blue-400">{scenario.scenario} Scenario</h3><div className="text-sm text-slate-400 mt-1">Probability: {scenario.probability}%</div></div><div className="text-right"><div className="text-2xl font-bold text-green-400">${scenario.year2030}M</div><div className="text-xs text-slate-400">2030 Revenue</div></div></div>
                                  <div className="grid grid-cols-4 gap-4 mb-4"><div className="text-center"><div className="text-lg font-bold text-cyan-400">${scenario.year2025}M</div><div className="text-xs text-slate-400">2025</div></div><div className="text-center"><div className="text-lg font-bold text-cyan-400">${scenario.year2026}M</div><div className="text-xs text-slate-400">2026</div></div><div className="text-center"><div className="text-lg font-bold text-cyan-400">${scenario.year2027}M</div><div className="text-xs text-slate-400">2027</div></div><div className="text-center"><div className="text-lg font-bold text-green-400">${scenario.year2030}M</div><div className="text-xs text-slate-400">2030</div></div></div>
                                  <div className="text-sm text-slate-300"><strong className="text-white">Key Assumptions:</strong><ul className="mt-2 space-y-1 ml-4">{scenario.assumptions.map((assumption, idx) => (<li key={idx}>• {assumption}</li>))}</ul></div>
                                </div>
                              ))}
                            </div>
                            <ResponsiveContainer width="100%" height={400}>
                              <LineChart>
                                <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                                <XAxis dataKey="year" stroke="#9ca3af" type="category" allowDuplicatedCategory={false} />
                                <YAxis stroke="#9ca3af" label={{ value: 'Revenue ($M)', angle: -90, position: 'insideLeft', fill: '#9ca3af' }} />
                                <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #475569', borderRadius: '8px' }} formatter={(value) => `${value}M`} />
                                <Legend />
                              </LineChart>
                            </ResponsiveContainer>
                            <div className="mt-6 bg-blue-900/20 border border-blue-700/50 rounded-lg p-4"><div className="font-semibold text-blue-400">Scenario Analysis Methodology:</div></div>
                          </div>
                        )}
                        ...
                        {selectedChart === 'evidence' && (
                          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                            <h2 className="text-2xl font-bold mb-6 text-cyan-400">📚 Complete Evidence Library</h2>
                            <div className="mb-6 bg-blue-900/20 border border-blue-700/50 rounded-lg p-4"><p className="text-slate-300">This library contains all source materials used in the financial analysis and charts.</p></div>
                            <div className="space-y-6">
                              {evidenceLibrary.map((category, catIdx) => (
                                <div key={catIdx} className="bg-slate-900/50 border border-slate-700 rounded-lg p-6">
                                  <h3 className="text-xl font-semibold text-blue-400 mb-4 flex items-center gap-2">{category.category}<span className="ml-auto text-sm text-slate-400">{category.sources.length} sources</span></h3>
                                  <div className="grid gap-3">{category.sources.map((source, srcIdx) => (
                                    <a key={srcIdx} href={source.url} target="_blank" rel="noopener noreferrer" className="block bg-slate-800/50 hover:bg-slate-700/50 border border-slate-600 hover:border-cyan-500 rounded-lg p-4 transition-all group">
                                      <div className="flex items-start justify-between gap-4"><div className="flex-1"><div className="font-semibold text-cyan-400 group-hover:text-cyan-300 mb-2 flex items-center gap-2">{source.title}</div><p className="text-sm text-slate-300 mb-2">{source.description}</p><div className="text-xs text-slate-500 font-mono truncate">{source.url}</div></div><div className="text-2xl opacity-50 group-hover:opacity-100 transition-opacity">🔗</div></div>
                                    </a>
                                  ))}</div>
                                </div>
                              ))}
                            </div>
                            <div className="mt-8 grid md:grid-cols-2 gap-4"><div className="bg-green-900/20 border border-green-700/50 rounded-lg p-4"><h4 className="font-semibold text-green-400 mb-2">✅ Data Verification Process</h4></div><div className="bg-amber-900/20 border border-amber-700/50 rounded-lg p-4"><h4 className="font-semibold text-amber-400 mb-2">⚠️ Data Limitations</h4></div></div>
                            <div className="mt-6 text-center"><button onClick={() => { const allUrls = evidenceLibrary.flatMap(cat => cat.sources.map(s => s.url)).join('\n'); navigator.clipboard.writeText(allUrls); }} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg transition-all font-semibold">📋 Copy All URLs to Clipboard</button><p className="text-xs text-slate-400 mt-2">Copies all {evidenceLibrary.reduce((sum, cat) => sum + cat.sources.length, 0)} source URLs</p></div>
                          </div>
                        )}
                      ...
                      <div className="max-w-7xl mx-auto mt-8 bg-slate-800/30 border border-slate-700 rounded-xl p-6">
                        <h3 className="text-xl font-semibold mb-4 text-cyan-400">🤖 AI-Ready Data Structures</h3>
                        <div className="grid md:grid-cols-2 gap-4 text-sm">
                          <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700"><div className="font-semibold text-blue-400 mb-2">Chart Data Exports Available:</div><ul className="space-y-1 text-slate-300"><li>• revenueData</li><li>• marketSizeData</li><li>• serviceMixData</li><li>• projectPipeline</li><li>• competitiveMetrics</li><li>• financialScenarios</li></ul></div>
                          <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700"><div className="font-semibold text-green-400 mb-2">Data Quality Attributes:</div><ul className="space-y-1 text-slate-300"><li>• <strong>source:</strong> Attribution</li><li>• <strong>confidence:</strong> High/Medium/Low</li><li>• <strong>note:</strong> Context</li></ul></div>
                        </div>
                      </div>
                    </div>
                  );
